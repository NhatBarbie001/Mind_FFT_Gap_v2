    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, r=4, only_kv=False,mlp=False):
        ...
        if mlp:
            self.adapter_mlp = nn.Sequential(OrderedDict([
                ("c_fc", nn.Linear(d_model, r, bias=False)),
                ("gelu", QuickGELU()),
                ("c_proj", nn.Linear(r, d_model, bias=False))
            ]))
            nn.init.zeros_(self.adapter_mlp.c_proj.weight)
        self.mlp_flag = mlp
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
...
def convert_weights_lora_model(model: nn.Module):
    """Convert applicable model parameters to fp16"""
    ...
    def _convert_weights_to_fp16(l):
        ...
        if isinstance(l, lora.MultiheadAttention):
            for attr in [*[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]], "in_proj_bias", "bias_k", "bias_v", *[f"{s}_proj_weight_lora_A" for s in ["in", "q", "k", "v"]], *[f"{s}_proj_weight_lora_B" for s in ["in", "q", "k", "v"]]]:
                tensor = getattr(l, attr)
                if tensor is not None:
                    tensor.data = tensor.data.half()
...
def build_LoRA_model(state_dict: dict, r: int, lora_mode: str):
    ...
    if "only_kv" in lora_mode:
        for name, param in list(state_dict.items()):
            if "transformer.resblocks." in name and 'in_proj_weight' in name:
                # pdb.set_trace()
                shape_chunk = param.shape[0]//3
                state_dict[name.replace("in_proj_weight", "q_proj_weight")] = param[:1*shape_chunk].clone()
                state_dict[name.replace("in_proj_weight", "k_proj_weight")] = param[1*shape_chunk:2*shape_chunk].clone()
                state_dict[name.replace("in_proj_weight", "v_proj_weight")] = param[2*shape_chunk:3*shape_chunk].clone()
                del state_dict[name]
            # if "transformer.resblocks" in name and 'in_proj_weight' in name:
            #     shape_chunk = param.shape[0]//3
            #     state_dict[name.replace("in_proj_weight", "q_proj_weight")] = param[:1*shape_chunk].clone()
    ...
    new_state_dict["lora_text_projection.weight"] = state_dict["text_projection"].T
    # pdb.set_trace()
    ...
    for name, param in model.named_parameters():
        ...
        #     print(name)
        # pdb.set_trace()
        if 'lora_text_projection' in name:
    ...
    ##
    ## ---------------------------------------------------------
    # convert_weights_lora_model(model)
    return model.eval()